...
# Предотвращение утечки токена в логах
...
# --- Команда /report — кнопки в теме "Сколько написал" ---
async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("💜 День тревоги и грусти", callback_data="mood_purple")],
        [InlineKeyboardButton("💙 День отдыха", callback_data="mood_blue")],
        [InlineKeyboardButton("💛 Слишком много работы!", callback_data="mood_yellow")],
        [InlineKeyboardButton("💚 День работы над текстом", callback_data="mood_green")],
        [InlineKeyboardButton("🤍 День работы над миром", callback_data="mood_white")]  # ← Новая кнопка
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Как прошёл ваш день?",
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("mood_"):
        if chat_id != GROUP_CHAT_ID or thread_id != THREAD_INPUT:
            await query.edit_message_text(
                "❌ Отметьте день только в теме *«Сколько написал»*.",
                parse_mode="Markdown")
            return
    elif query.data in ["stats_week", "stats_month"]:
        # Для статистики разрешаем любой чат
        pass  # не блокируем
    else:
    ...
    mood_map = {
        "mood_purple": ("💜", "День тревоги и грусти"),
        "mood_blue": ("💙", "День отдыха"),
        "mood_yellow": ("💛", "Слишком много работы!"),
        "mood_green": ("💚", "День работы над текстом"),
        "mood_white": ("🤍", "День работы над миром")
    }
    ...
    if query.data in mood_map:
        ...
        stats[user_key]["moods"][today] = query.data.split("_")[
            1]  # purple/blue/yellow/green
        save_stats(stats)
        ...
        if query.data == "mood_green":
            ...
            sent = await query.edit_message_text(
                "📝 Введите количество написанных символов (например: 2500, 5,3к):"
            )
            user_states[user_id]["message_id"] = sent.message_id

            job = context.job_queue.run_once(timeout_handler,
                                             when=300,
                                             data={
                                                 "user_id": user_id,
                                                 "username": username
            ...
            await query.edit_message_text(
                f"✅ Вы отметили: {emoji} Сегодня у вас {desc}\nСпасибо за честность! 💖"
            )
        ...
        now = datetime.now()
        period_ago = now - (timedelta(
            days=7) if query.data == "stats_week" else timedelta(days=30))
        title = "📊 Статистика за неделю" if query.data == "stats_week" else "📈 Статистика за месяц"

        total = sum(count for d, count in entries.items()
                    if datetime.strptime(d, "%Y-%m-%d") >= period_ago)
        days = sum(1 for d in entries
                   if datetime.strptime(d, "%Y-%m-%d") >= period_ago)
        ...
        try:
            ...
            await query.edit_message_text(
                "❗ Не удалось отправить в ЛС. Напишите боту: @Sterladometr_bot"
            )
...
async def timeout_handler(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=user_id,
            text=
            "⏰ Прошло 5 минут без ответа. Вам засчитано *100 символов* за день.",
            parse_mode="Markdown")
    except:
        pass
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if count is None:
        await update.message.reply_text(
            "❌ Не удалось распознать число. Примеры: 2500, 5,3к, 2.7k, 1.5 тыс."
        )
        return
    ...
    action = "перезаписано" if today in stats[user_key][
        "entries"] else "засчитано"
    await update.message.reply_text(f"✅ {action}: *{count:,}* символов!",
                                    parse_mode="Markdown")
...
def generate_report(stats, start_date, end_date, title):
    ...
    for user_id, user_data in stats.items():
        name = user_data["name"]
        chars = sum(c for d, c in user_data["entries"].items()
                    if start_date <= d <= end_date)
        green = sum(1 for d, m in user_data["moods"].items()
                    if start_date <= d <= end_date and m == "green")
        blue = sum(1 for d, m in user_data["moods"].items()
                   if start_date <= d <= end_date and m == "blue")
        yellow = sum(1 for d, m in user_data["moods"].items()
                     if start_date <= d <= end_date and m == "yellow")
        purple = sum(1 for d, m in user_data["moods"].items()
                     if start_date <= d <= end_date and m == "purple")
        white = sum(1 for d, m in user_data["moods"].items()
                    if start_date <= d <= end_date and m == "white")
...
async def top_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id, user_data in stats.items():
        ...
        for date_str, count in user_data["entries"].items():
        ...
        if total_chars > 0:
            char_count[user_id] = (name, total_chars)
        if active_days > 0:
            day_count[user_id] = (name, active_days)

    top_chars = sorted(char_count.items(), key=lambda x: x[1][1],
                       reverse=True)[:5]
    top_days = sorted(day_count.items(), key=lambda x: x[1][1],
                      reverse=True)[:5]

    lines = ["🏆 *ТОП-5 за неделю*"]

    lines.append("\n🖋 Больше всех написали:")
    if top_chars:
        for i, (uid, (name, chars)) in enumerate(top_chars, 1):
            lines.append(f"{i}. {name} — *{chars:,}* зн.")
    else:
        lines.append("Никто ещё не писал.")

    lines.append("\n📅 Чаще всего писали:")
    if top_days:
        for i, (uid, (name, days)) in enumerate(top_days, 1):
            lines.append(f"{i}. {name} — *{days}* дн.")
    else:
        lines.append("Никто ещё не писал.")
...
def main():
    ...
    weekly_time = Time(hour=WEEKLY_REPORT_HOUR,
                       minute=0,
                       second=0,
                       tzinfo=moscow)
    monthly_time = Time(hour=MONTHLY_REPORT_HOUR,
                        minute=0,
                        second=0,
                        tzinfo=moscow)
    ...
    app.run_polling()
    print(f"Время отчёта: {weekly_time} (часовой пояс: {weekly_time.tzinfo})")
    print(f"Сегодня в МСК: {datetime.now(moscow).weekday()}")
